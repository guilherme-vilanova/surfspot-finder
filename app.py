...
def beach_map_cache_get(key):
    return layered_cache_get(BEACH_MAP_CACHE_NAMESPACE, beach_map_cache, key)


def beach_map_cache_set(key, value):
    layered_cache_set(BEACH_MAP_CACHE_NAMESPACE, beach_map_cache, key, value, SEARCH_CACHE_TTL)
...
def build_beach_map_query(beach):
    ...
    if region and state:
        return f"{beach['name']}, {region}, {state}, {BRAZIL_LABEL}"
    if state:
        return f"{beach['name']}, {state}, {BRAZIL_LABEL}"
    return f"{beach['name']}, {BRAZIL_LABEL}"


def resolve_beach_map_target(beach):
    cache_key = ((beach.get("name") or "").casefold(), (beach.get("region") or "").casefold())
    cached = beach_map_cache_get(cache_key)
    if cached is not None:
        enriched = beach.copy()
    ...
    try:
        resolved = location_service.find_place(
            build_beach_map_query(beach),
            lat=beach.get("lat"),
            lon=beach.get("lon"),
        )
        resolved_map = {
            "map_lat": resolved["lat"],
            "map_lon": resolved["lon"],
            "map_place_id": resolved.get("place_id"),
        }
    except LocationServiceError:
        try:
            resolved = location_service.geocode_address(build_beach_map_query(beach))
            resolved_map = {
                "map_lat": resolved["lat"],
                "map_lon": resolved["lon"],
                "map_place_id": resolved.get("place_id"),
    ...
    beach_map_cache_set(cache_key, resolved_map)

    enriched = beach.copy()
    enriched.update(resolved_map)
    return enriched


def build_beach_map_embed_url(beach):
    api_key = get_google_maps_api_key()
    map_place_id = beach.get("map_place_id")
    map_lat = beach.get("map_lat", beach["lat"])
    map_lon = beach.get("map_lon", beach["lon"])
    if api_key and map_place_id:
        return (
            "https://www.google.com/maps/embed/v1/place"
            f"?key={quote_plus(api_key)}&q={quote_plus(f'place_id:{map_place_id}')}&zoom=12"
        )
    return f"https://maps.google.com/maps?q=loc:{map_lat},{map_lon}&z=12&output=embed"
...
def build_beach_google_maps_url(beach):
    ...
    map_lon = beach.get("map_lon", beach["lon"])
    if map_place_id:
        query = build_beach_map_query(beach)
        return (
            "https://www.google.com/maps/search/?api=1"
            f"&query={quote_plus(query)}&query_place_id={quote_plus(map_place_id)}"
        )
    return f"https://www.google.com/maps/search/?api=1&query={map_lat},{map_lon}"
